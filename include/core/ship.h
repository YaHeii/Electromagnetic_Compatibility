...
{
public:
    ship(string id, utils::Point2D location, vector<double> distance = {}, double orientation_deg = 30.0, double speed = 1.0);
    double getOrientationDeg() const { return m_orientation_deg; } // 船体朝向，0度为X轴正向
    const std::string& getID() const { return m_id; }

    utils::Point2D getLocation() const {
    return m_location;
    }
    ...
    //设置坐标
    void setLocation(utils::Point2D position_new);

private:
    string m_id;
    utils::Point2D m_location;
