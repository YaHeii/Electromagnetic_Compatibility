...
namespace core {
    ...
    EMCEngine::EMCEngine(const models::PropagationModel& prop_model)
    : m_prop_model(prop_model) {}
    std::vector<InterferenceResult> EMCEngine::analyzeFleet(const core::Fleet& fleet) {//返回受扰计算结果数组
        std::vector<InterferenceResult> results;

        const auto& ships = fleet.getShips();
        std::vector<std::pair<core::Transmitter*, core::ship*>> all_transmitters;
        std::vector<std::pair<core::Receiver*, core::ship*>> all_receivers;

        // 存储所有船的接收机与发射机
        for (const auto& ship_ptr : ships) {
            for (const auto& eq_ptr : ship_ptr->getEquipmentList()) {
                if (auto tx = dynamic_cast<core::Transmitter*>(eq_ptr.get())) {
                    all_transmitters.push_back({tx, ship_ptr.get()});
                } else if (auto rx = dynamic_cast<core::Receiver*>(eq_ptr.get())) {
        ...
        for (const auto& rx_pair : all_receivers) {
            ...
            core::ship* victim_ship = rx_pair.second;//定义受害船
            ...
            utils::Point2D victim_rx_global_pos = victim_ship->getLocation();//计算设备的全局位置
            ...
            for (const auto& tx_pair : all_transmitters) {
                core::Transmitter* aggressor_tx = tx_pair.first;//发射设备
                core::ship* aggressor_ship = tx_pair.second;//发射船
                ...
                //攻击船坐标计算
                utils::Point2D aggressor_tx_global_pos = aggressor_ship->getLocation();
                ...
                if (is_potentially_interfering) {
                    ...
                    total_interference_linear_watts += utils::dbmToWatts(received_power_dbm);
            ...
            // 计算最终结果
            double total_interference_dbm = utils::wattsToDbm(total_interference_linear_watts);
            double noise_floor_dbm = victim_rx->getNoiseFloorDBm();

            //计算I+N，单位watt
            double total_interference_plus_noise_watts = total_interference_linear_watts + utils::dbmToWatts(noise_floor_dbm);
            //计算S
            ...
            double S_watts = utils::dbmToWatts(S_db);//转换为瓦特
            //计算SINR
            double SINR_watts = S_watts / total_interference_plus_noise_watts;
            double SINR_db = utils::wattsToDbm(SINR_watts);


            //干扰裕度(IM)=灵敏度-(I+N)
            //正互调意味着即使有干扰和噪声，信号仍高于灵敏度。
            //阴性IM表示低于灵敏度，可能不起作用。
            double total_interference_plus_noise_dbm = utils::wattsToDbm(total_interference_plus_noise_watts);
