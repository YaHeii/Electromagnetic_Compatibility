...
PathManager::PathManager(int max_num_path, std::vector<Path> Path_list) {
    this->max_num_path = max_num_path;
    this->Path_list = Path_list;
}

Path PathManager::Id_findPath(std::string ship_id, std::vector<Path> path_list) const {
...
// 获取指定时间的路径算法
Path PathManager::getPathsInTimeRange(int time_index, std::vector<Path> path_list) const {
    for (const auto& path : path_list) {
        // 检查时间范围是否有重叠
...
// 判断指定时间是否有路径数据
bool PathManager::hasPathAtTime(int time) const {
    try {
...
void PathManager::setMaxNumPath(int max_num_path) {
    this->max_num_path = max_num_path;
}